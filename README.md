...
トップページの Instagram 欄 2 か所は、**6 時間おきに最新投稿を取り込んで自動更新** します
...
| 欄 | アカウント | シークレット名 |
|---|---|---|
| 0歳〜100歳のおいもフォト (4枚) | @oimo.photo | `INSTAGRAM_TOKEN_PHOTO` |
| Instagram (7枚) | @oimo.fes | `INSTAGRAM_TOKEN` |

Instagram API は**トークンの持ち主の投稿しか返さない**ため、アカウントごとにトークンが要ります。
**動かすにはアクセストークンの登録が 1 回だけ必要です。** 未登録のあいだは、
アカウントへのリンクだけが表示されます (エラーにはなりません)。
片方だけ登録した場合は、その欄だけが更新されます。
どちらのアカウントも**プロアカウント**にしたうえで、それぞれ下の手順でトークンを取ります
(同じ Meta アプリを使い回せます。アカウントごとに「Instagramテスター」の招待と承認が要ります)。
